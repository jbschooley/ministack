...
### Added
- **Aurora DSQL — foreign key constraints** — every `REFERENCES` / `FOREIGN KEY` clause was refused `0A000`, so a schema with referential integrity could not be created at all; Aurora DSQL has supported foreign keys since 2026-08-26. They now reach the backend as written — the five referential actions, `MATCH FULL` / `MATCH SIMPLE`, self-references, composite keys and deferrability — with the two DSQL-specific rules enforced, measured against a live cluster (eu-central-1, 2026-08-30): `ALTER TABLE ... ADD CONSTRAINT ... FOREIGN KEY` must use `NOT VALID` (as `CHECK` already did, with `ALTER TABLE ASYNC ... VALIDATE CONSTRAINT` validating it as a job afterwards), and `DEFERRABLE` is refused on any other kind of constraint, in the service's three wordings (`DEFERRABLE constraint not supported`, `CHECK constraints cannot be marked DEFERRABLE`, and `ALTER TABLE / ADD CONSTRAINT UNIQUE / DEFERRED / INITIALLY DEFERRED not supported`), DSQL restricting it to foreign keys; `NOT DEFERRABLE` and `INITIALLY IMMEDIATE` declare no deferral and are accepted anywhere. Referential integrity is the backend's, so a referencing write racing a change to the referenced key waits on a row lock instead of failing the loser with `40001` (OC000). Contributed by @vivedo.
...
- **Aurora DSQL — change data capture streams, and `GetVpcEndpointServiceName`** — `CreateStream` and the three other CDC operations answered `ValidationException` (`Unknown path`), so an application that provisions a stream alongside its cluster could not be pointed at MiniStack at all; CDC went GA on 2026-07-08. Streams now create, describe, list, page, tag and delete with the API's shapes and defaults — `clientToken` idempotency, the 5-per-cluster quota as `ServiceQuotaExceededException` (`You have reached the stream limit.`), and a cluster's streams going with it on delete. Metadata only: no change record reaches the Kinesis target, so a stream is `ACTIVE` on creation unless its target is one MiniStack can see is broken — an absent Kinesis stream, or a role that cannot be assumed under `AUTH=true` — which lands `FAILED` carrying the service's `statusReason` error code. `GetVpcEndpointServiceName`, missing since the emulator landed, answers a stable per-cluster `com.amazonaws.<region>.dsql-<suffix>`. Contributed by @vivedo.
- **IoT Core — `ListThingGroupsForThing`** — `GET /things/{thingName}/thing-groups` answered `400 InvalidRequestException: Unsupported IoT path`, so resolving which groups a thing belongs to meant running `ListThingsInThingGroup` over every group; the forward lookup existed, the reverse one did not. The membership store was already bidirectional, so the operation now reads the thing record's group names and returns `{groupName, groupArn}` pairs, with `ResourceNotFoundException` for an unknown thing. The full list comes back in one page — `maxResults` / `nextToken` are deliberately not honored, like the other IoT list operations here. Contributed by @iot-rocket.

### Fixed
- **Aurora DSQL — the `ALTER TABLE` refusals answer what the service answers** — measuring the emulator against a live cluster (eu-central-1, 2026-08-30) found four statements answered differently. A refused `ADD CONSTRAINT` — `PRIMARY KEY`, an inline `UNIQUE`, `EXCLUDE`, or a `CHECK`/`FOREIGN KEY` without `NOT VALID` — drew three invented messages where DSQL answers one, `unsupported ALTER TABLE ADD CONSTRAINT statement`. `ADD COLUMN` refused only `DEFAULT` and `NOT NULL` and let `REFERENCES`, `UNIQUE`, `CHECK`, `GENERATED ... STORED` and an inline identity through, where DSQL takes `name type [STORAGE mode]` and nothing else (`ALTER TABLE ADD COLUMN with constraint not supported`, a bare `NULL` apart, and `COLLATE clause not supported` for a collation); `ALTER COLUMN ... ADD GENERATED AS IDENTITY` on an existing column is unaffected. `ALTER TABLE ASYNC ... VALIDATE CONSTRAINT` over rows that break the constraint relayed the backend's `23503` at submit time, where DSQL hands back a `job_id` and fails the job — `sys.jobs` now reports `failed` with the violation as `details` and `sys.wait_for_job` answers false. Adding several columns in one statement also read the second action as part of the first column's type. Contributed by @vivedo.
- **Cognito — `AdminListUserAuthEvents` requires user-pool add-ons, as real Cognito does** — the call answered `{"AuthEvents": []}` for every pool, but AWS refuses it with `UserPoolAddOnNotEnabledException` (400) unless the pool's `UserPoolAddOns.AdvancedSecurityMode` is enabled; the permissive empty list let caller bugs an add-ons check would surface — such as indexing into the events unguarded — pass locally and fail against real AWS. The stored `UserPoolAddOns` now gates the operation; with add-ons enabled the answer stays an empty list, since auth events are never recorded. Contributed by @iot-rocket.
...
- **AppSync — `util.transform` DynamoDB expression builders** — DynamoDB is the data source most AppSync APIs use, and `toDynamoDBFilterExpression` is how a resolver turns a GraphQL filter argument into something DynamoDB can evaluate, so of the four transform functions it is the one that gets called; it returned nothing, because `util.transform` was an empty object. It now builds the `{expression, expressionNames, expressionValues}` triple over AWS’s documented operator set — `eq`, `ne`, `lt`, `le`, `gt`, `ge`, `contains`, `notContains`, `beginsWith`, `between`, `in`, `attributeExists`, `attributeType` — with `and`/`or`/`not` nesting and sibling conditions ANDed. `toDynamoDBConditionExpression` shares it, being the same grammar. Contributed by @jbschooley.
- **AppSync — the rest of the `util` surface** — 8 of the 21 members `@aws-appsync/utils` declares were provided, and three of those were wrong rather than incomplete: `autoUlid` returned a UUID, `util.dynamodb` had a single pass-through `toDynamoDB` that produced no AttributeValue shapes, and `util.transform` was an empty object so a call failed as “not a function” away from the cause. Adds `base64Encode`/`Decode`, `urlEncode`/`Decode`, `escapeJavaScript`, `matches`, `authType`, real `autoUlid` and `autoKsuid`, `util.str.normalize`, all of `util.math`, the six missing `util.time` functions, the full `util.dynamodb` `to*` family, and `util.rds.toJsonObject`. `util.xml`, `util.transform` and the `util.http` response-header functions throw naming themselves, since they need an XML parser, a DynamoDB expression builder, or the response AppSync is building. Contributed by @jbschooley.
- **AppSync — the `@aws-appsync/utils/dynamodb` helpers, and `EvaluateCode`** — the helper sub-module was stripped along with every other import, which is right for the types-only root package but wrong here: `ddb.get`/`put`/`update`/`remove`/`scan`/`query` build the request objects a resolver returns, so a resolver using them died with “ddb is not defined”. They are now provided and bound to whatever name the import used. Fixing that surfaced a second bug — the compile cache re-ran the module on every call, throwing on any top-level `const` a resolver declares — so a module is now evaluated once with the injected globals closing over a mutable holder. `EvaluateCode` is implemented on top of the evaluator, so a resolver can be tested before it is attached to an API, as it can against AWS. Contributed by @jbschooley.
- **AppSync — nested type resolvers, `ctx.identity` from Cognito, `ctx.env` and `ctx.error`** — only top-level `Query`/`Mutation` fields resolved, so a resolver on a nested type never fired and its field was simply absent from the response, which is where most of an API’s resolvers live. A resolved value’s type is now looked up in the API’s SDL and the resolvers attached to it run with the parent as `ctx.source`. `ctx.identity` is populated from a Cognito token when the API is configured for a user pool — it was only ever set by a Lambda authorizer, so a Cognito API saw `null` and every permission check failed. `ctx.env` exposes the API’s environment variables, and `ctx.error` is set when the data source fails so `response()` can turn an upstream failure into its own error or a fallback. Contributed by @jbschooley.
- **AppSync — `APPSYNC_JS` resolvers execute** — resolver code was stored and never run: the executor dispatched on data source type and inferred an operation from the field arguments, so a resolver’s own logic never affected the answer, and four of the six data source types the control plane accepts fell through to a mock that echoed the arguments. `request()` now decides what the data source is asked for and `response()` shapes the answer, with `util.error` becoming a GraphQL error carrying its `errorType`, `util.appendError` riding alongside the data, and `runtime.earlyReturn` following what AWS documents in both places it can appear — from a function the data source and response handler are skipped and the next function runs, from a pipeline resolver the pipeline is skipped and the resolver’s own response handler still runs. Pipeline resolvers run their functions in order with `ctx.stash` carried across, and response handlers see both `ctx.prev.result` and `ctx.result`. All three `APPSYNC_JS` globals are provided, `extensions` included: its `evictFromApiCache` calls are recorded and handed back to the service, which is how a mutation invalidates a cached query. A resolver returning `{operation: "Invoke", payload}` sends that payload to a Lambda data source verbatim. Data source handlers and runtimes are registries, so `NONE`, `HTTP`, `AMAZON_DYNAMODB` and `AWS_LAMBDA` are entries rather than branches and the two remaining types refuse with `NotImplemented` instead of answering a mock. A resolver with no `APPSYNC_JS` code behaves exactly as before. No new dependency — evaluation reuses the Node the image already ships for the `nodejs*` Lambda runtimes. Note that Node is more permissive than the real `APPSYNC_JS` sandbox, so a resolver that runs here can still be refused on deploy. Contributed by @jbschooley.
...
- **AppSync — the API cache, control plane and data plane** — `CreateApiCache`, `GetApiCache`, `UpdateApiCache`, `DeleteApiCache` and `FlushApiCache` all answered `Unsupported route`, so `aws_appsync_api_cache` could not be applied and an environment that enables caching in the cloud had to turn it off locally, diverging from the thing it is meant to reproduce. The five operations now round-trip a cache record — one per API, released with its API, surviving a restore — with `atRestEncryptionEnabled` and `transitEncryptionEnabled` fixed at create time and carried across an update rather than reverting to false. The data plane honours it as well, which the control plane alone did not: query resolvers consult the cache, following AppSync’s behaviour for `PER_RESOLVER_CACHING` and `FULL_REQUEST_CACHING`, with caching keys evaluated against the field’s arguments, identity and source, and a key that cannot be resolved disabling caching for that call rather than collapsing two callers onto one entry. Mutations neither read nor populate it, only successful results are cached, and `FlushApiCache` empties it. Contributed by @jbschooley.
- **AppSync — schema creation, pipeline functions, environment variables and the `Update*` operations** — `StartSchemaCreation`, `GetSchemaCreationStatus`, `GetIntrospectionSchema`, the function operations, `Put`/`GetGraphqlApiEnvironmentVariables` and `UpdateDataSource` / `UpdateResolver` / `UpdateFunction` / `UpdateType` all answered `Unsupported route`, so an API could be created but had no schema, no pipeline functions, and nothing about it could be changed in place. `CreateGraphqlApi` also omitted `apiType`, `visibility` and `introspectionConfig` — the first two `ForceNew` in the Terraform provider, so a no-change plan proposed replacing the API and every resource beneath it — and left the tags it was given off the API object, where the provider reads them. The SDL is stored as given and served back verbatim rather than parsed. Contributed by @jbschooley.

### Fixed
- **RDS — a cluster endpoint is a stable name, not the container's current address** — the endpoint reported for a cluster changed as the cluster came up: an AWS-shaped hostname before an instance existed, then `localhost`, then the container's IP. Anything that read it got a value that depended on when it read, and Terraform reads once, during the apply that creates the cluster — so a function given the endpoint through its environment was handed a name nothing resolves, and failed at `getaddrinfo` rather than at connect. The address then went stale whenever the container was replaced, which on a restore could hand a consumer an address since reassigned to something else. The endpoints the cluster already advertises — writer and, where reads cannot move, reader — are now registered as Docker network aliases on its container and reported unchanged, so the values handed out at create time keep resolving, including after the container comes back at a different address. Skipped where Docker does not allow aliases (the default bridge), which behaves exactly as before. Contributed by @jbschooley.
...
- **ECS — a task runs on the architecture its task definition declares** — `runtimePlatform` was stored on the task definition and never read again, so Docker chose the host's architecture. An `ARM64` task definition on an x86_64 host started a container that could not execute its own entrypoint, and the task still reported as started, because creating the container is the part that succeeds — the failure surfaced only as a service that never answered. The declared architecture is now passed to Docker; a task definition without a `runtimePlatform` is unchanged. Contributed by @jbschooley.
- **Lambda — a function runs on the architecture it declares** — the container was created without a platform, so Docker used the host's architecture whatever `Architectures` said. An arm64 function on an x86_64 host ran as x86_64, which is invisible until a layer carries a native wheel: the handler then dies at import naming the library rather than the mismatch (`no pq wrapper available`, from psycopg loading its own binary). The declared architecture is now passed to Docker, and a cached image of the wrong architecture is re-pulled for the right one rather than failing opaquely at run. Contributed by @jbschooley.
