...
- **AppSync — optional enforcement of an API’s auth modes (`APPSYNC_ENFORCE_AUTH=1`)** — an API declaring `AMAZON_COGNITO_USER_POOLS` served a caller with no credentials at all, so an authorization test written against MiniStack passed regardless of what the resolvers did. With the variable set, a request satisfying none of the API’s configured providers answers `401 UnauthorizedException`. Credentials are still not verified — MiniStack has no authentication of its own by design — what is refused is the absence of any credential. Off by default, since enforcement changes behaviour an existing suite may depend on. Contributed by @jbschooley.
...
- **AppSync — queries are parsed and executed with `graphql-core`** — the data plane picked queries apart with regular expressions, which works for the shapes it was written against and fails for everything else: aliases, fragments, nested selection sets, variables with defaults, directives. It failed by returning null rather than by refusing, so a query ministack could not parse was indistinguishable from a resolver that returned nothing. The SDL is now built into a real schema, the document is parsed and validated against it before any resolver runs, and execution walks the selection set calling the resolver for each field — so an invalid query is a validation error, as it is on AWS. An AppSync prelude supplies the scalars and directives every AppSync schema assumes (`AWSDate`, `AWSDateTime`, `AWSJSON`, `AWSEmail`, `AWSURL` and the `@aws_*` authorization directives) so a schema written for AppSync builds unmodified, and schemas are cached per API. Adds `graphql-core` as a dependency. Contributed by @jbschooley.
...
- **Persistence — a module that failed to load no longer overwrites its state** — when a service module cannot be imported (a missing dependency, a syntax error) it is replaced by a stand-in that answers `ServiceUnavailable`. The stand-in reported empty state and `save_all` wrote it over the service’s file, so one bad boot silently destroyed everything that service had persisted. The stand-in now reports `None` and the file is left alone; a service that loaded and genuinely holds nothing still writes, since that is a real state. Contributed by @jbschooley.
- **AppSync — an anonymous operation that declares variables is parsed** — `mutation($input: CreateThingInput!) { … }`, with no space between the keyword and the parenthesis, is what every SDK and generated client sends. The operation pattern required whitespace after the keyword, so it never matched and the bare-field fallback read the whole document as a single field literally named “mutation”, answering `{"data": {"mutation": null}}` with no error to explain it. Every mutation from a real client behaved that way. Contributed by @jbschooley.
- **AppSync — resolver execution no longer blocks the event loop** — executing a resolver can call back into MiniStack (an HTTP data source in front of its own API Gateway or load balancer, a Lambda data source invoking a function it hosts), and the execution path blocked inside an async handler, so the nested request could never be served and the call timed out. Execution now runs on a worker thread. Contributed by @jbschooley.
